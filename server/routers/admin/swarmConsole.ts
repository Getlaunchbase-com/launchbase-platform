...
async function computeProfileStats(
  ...
  profileId: number,
  window: "24h" | "7d" | "30d"
): Promise<z.infer<typeof ProfileStatsSchema>> {
  const since = new Date(Date.now() - windowToMs(window));
  const rows: any[] = await db
    .select({
      stopReason: swarmRuns.stopReason,
      costUsd: swarmRuns.costUsd,
      latencyMs: swarmRuns.latencyMs,
    })
    .from(swarmRuns)
    .where(and(eq(swarmRuns.profileId as any, profileId), gte(swarmRuns.createdAt, since)));
  ...
  for (const r of rows) {
    const sr = (r.stopReason || "unknown") as string;
    counts.set(sr, (counts.get(sr) ?? 0) + 1);
    if (typeof r.latencyMs === "number") lat.push(r.latencyMs);
    if (typeof r.costUsd === "number") cost.push(r.costUsd);
  }

  const ok = counts.get("ok") ?? 0;
  const tests_failed = counts.get("tests_failed") ?? 0;
  const patch_invalid = counts.get("patch_invalid") ?? 0;
  const other = runs - ok - tests_failed - patch_invalid;
  const passRate = runs ? ok / runs : 0;

  const avgCostUsd = cost.length ? cost.reduce((a, b) => a + b, 0) / cost.length : null;
  const p95LatencyMs = lat.length
    ? (() => {
...
export const adminSwarmConsoleRouter = router({
  ...
  repoSources: router({
    ...
    createLocal: adminProcedure.input(CreateRepoSourceLocalInputSchema).mutation(async ({ input }) => {
      ...
      const res = await db.insert(repoSources).values({
        name: input.name,
        type: "local",
        localPath: input.localPath,
      } as any);
      const insertId = Number((res as any)?.insertId ?? (res as any)?.[0]?.insertId ?? 0);
      const [row] = await db.select().from(repoSources).where(eq(repoSources.id, insertId)).limit(1);
      return row as any;
    }),
      ...
      if (featurePack.repoSourceId) {
        ...
        if (featurePack.sourceType === "manual" && (!featurePack.failurePacketJson || featurePack.failurePacketJson.trim().length < 5)) {
          ...
          const pkt = {
            ...
            testCommands: (featurePack.testCommands || []).map((tc: { cmd: string; args?: string[] }) => `${tc.cmd} ${(tc.args || []).join(" ")}`.trim()).filter(Boolean),
          };

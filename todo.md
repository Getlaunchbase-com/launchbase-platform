...
## 🤖 AGENT-STACK INTEGRATION & PRODUCTIZATION

**Status:** ✅ Integration Complete - Productization In Progress  
**Version:** 44c24771 (Agent-Stack Integration)

### Cleanup Tasks
- [ ] Remove `version: "3.9"` from agent-stack docker-compose.yml
- [ ] Add `uvicorn[standard]` to agent-stack router requirements.txt
- [ ] Rebuild router container and verify no WebSocket warnings

### Security Hardening
- [ ] Bind router to localhost only (`127.0.0.1:8080:8080`)
- [ ] Verify ROUTER_TOKEN is set and enforced
- [ ] Add rate limiting to router endpoints
- [ ] Document security configuration

### UI Enhancements
- [x] Add "Agent Stack" link to admin navigation sidebar
- [x] Create session picker component (create/select/delete sessions)
- [x] Add UI controls for browser_click tool
- [x] Add UI controls for browser_input tool
- [x] Add UI controls for browser_screenshot tool
- [ ] Implement "Record" mode for run logs (future: requires db migration)
- [ ] Add run history view (future: requires agent_run_history table)

### Production Polish
- [ ] Replace `sleep infinity` with proper entrypoint script
- [ ] Add healthchecks to browser and runner containers
- [ ] Create install script (setup .env, set token, bring up compose)
- [ ] Create upgrade script (pull, rebuild, smoke test)
- [ ] Add smoke test command (health + browser_goto)

### Documentation
- [ ] Document agent-stack API endpoints
- [ ] Document LaunchBase integration architecture
- [ ] Create deployment guide
- [ ] Create troubleshooting guide

### Testing
- [ ] Add tests for browser_click integration
- [ ] Add tests for browser_input integration
- [ ] Add tests for browser_screenshot integration
- [ ] Add end-to-end orchestration test


## 🚨 DEPLOYMENT FIX (URGENT)
- [x] Fix missing system_field_general.md in production build
- [x] Update build configuration to include markdown files
- [ ] Test deployment succeeds



## 🧠 MARKETING INTELLIGENCE - Enterprise Hybrid Signal Pipeline

**Architecture:** API-first with scrape fallback, GCP-native, pluggable ingestors  
**Pattern:** Fetch → Normalize → Dedupe → Score → Human Review → Enrich → Experiment

### Phase 1: Core Infrastructure (Signals Inbox)
- [x] `marketing_signals` table with normalized schema:
  - `id`, `source_type`, `jurisdiction`, `entity_name`, `event_date`
  - `canonical_key` (dedupe key with unique index)
  - `raw_json` (original payload)
  - `score`, `reasons` (scoring justification)
  - `status` (new|triaged|qualified|rejected|converted)
  - `assigned_to`, `notes`, `tags`
- [x] Drizzle schema migration
- [ ] Upsert logic with `canonical_key` dedupe
- [x] Marketing Inbox UI at `/admin/marketing-inbox` (using marketingInboxItem table temporarily)
- [ ] Migrate UI to use `marketing_signals` table
- [ ] Filters: status, source_type, jurisdiction, score range
- [ ] Actions: assign, change status, add notes/tags

### Phase 2: Delaware LLC Ingestor (First Signal Source)
- [x] `server/ingestors/llc/de_api.ts` - API-first collector
- [x] `server/ingestors/llc/de_scrape.ts` - Playwright fallback (uses agent-stack)
- [x] Canonical key generation: `sha256(source_type + jurisdiction + entity_name + event_date + source_native_id)`
- [x] Test seeder with sample Delaware LLCs
- [x] Manual trigger endpoint for testing (tRPC `marketingSignals.seedTestDelawareLLCs` and `marketingSignals.ingestDelawareLLCs`)

### Phase 3: GCP Collection Runners
- [ ] Cloud Scheduler → Pub/Sub → Cloud Run Job architecture
- [ ] Secret Manager integration for API keys
- [ ] Cloud Logging for ingestion audit trail
- [ ] Config-driven collector registry (enable/disable per source)
- [ ] Error handling + retry logic

### Phase 4: Enrichment & Scoring
- [ ] Domain lookup service
- [ ] Website scraper (tech stack detection)
- [ ] Social presence checker
- [ ] Deterministic scoring rules:
  - Intent signals (keywords, industry, timing)
  - Fit signals (size, location, tech stack)
  - Recency bonus
- [ ] Score reasons (explainability)

### Phase 5: Multi-Source Expansion
- [ ] California LLC ingestor (`ingestors/llc/ca_api.ts`, `ca_scrape.ts`)
- [ ] Texas LLC ingestor (`ingestors/llc/tx_api.ts`, `tx_scrape.ts`)
- [ ] USPTO trademark ingestor (`ingestors/trademark/uspto_api.ts`)
- [ ] Domain registration ingestor (`ingestors/domain/whois_api.ts`)
- [ ] Job posting ingestor (`ingestors/jobs/linkedin_scrape.ts`)

### Phase 6: Experiment Engine Integration
- [ ] Generate outreach/copy variants from qualified signals
...
- [ ] Load test for high-volume ingestion
- [ ] Monitoring dashboards (ingestion rate, error rate, score distribution)
- [ ] Alerting for ingestion failures



## 🎯 MARKETING SIGNALS - Concrete Implementation (User Spec)

### Phase 1: Schema Alignment
- [ ] Review current `marketing_signals` schema vs user's spec
- [ ] Add missing fields if needed: `entityType`, `externalId`
- [ ] Ensure indexes match: `uq_canonical_key`, `idx_source_type`, `idx_jurisdiction`, `idx_score`, `idx_status`, `idx_created_at`
- [ ] Verify `canonicalKey` generation uses correct format: `sha256(jurisdiction|sourceType|entityName)`

### Phase 2: tRPC Router Refinement
- [ ] Update `marketingSignalsRouter.list` to match user's filter spec (q, sourceType, jurisdiction, status, scoreMin, scoreMax, limit, offset)
- [ ] Ensure `seedTestDelawareLLCs` generates 10 test rows with proper canonical keys
- [ ] Verify `ctx.db` usage (not `getDb()`) in router procedures
- [ ] Add proper duplicate handling in seed (try/catch per row)

### Phase 3: Marketing Inbox UI
- [ ] Create `/admin/marketing-inbox` page with filters (search, status dropdown, score range)
- [ ] Add "Seed Test Signals" button
- [ ] Display table with columns: Created, Name, Jurisdiction, Source, Score, Status
- [ ] Wire to `trpc.marketingSignals.list` and `trpc.marketingSignals.seedTestDelawareLLCs`
- [ ] Add navigation link in admin sidebar

### Phase 4: End-to-End Testing
- [ ] Visit `/admin/marketing-inbox`
- [ ] Click "Seed Test Signals"
- [ ] Verify 10 rows appear in table
- [ ] Test search filter
- [ ] Test status filter
- [ ] Verify duplicate seeds are ignored (canonical key dedupe works)

### Phase 5: GCP Collection Runner (Future)
- [ ] Design Cloud Scheduler → Pub/Sub → Cloud Run Job architecture
- [ ] Collector writes to DB directly or calls LaunchBase tRPC endpoint (token-protected)
- [ ] Keep collection separate from review workflow



## 🔄 MIGRATION STRATEGY - Old Inbox → New Signals Inbox

### Phase A: Ship Side-by-Side (No Breakage)
- [ ] Keep `marketingInboxItem` and `marketingRunLog` tables (do not delete yet)
- [ ] Keep `marketingInboxRouter` mounted (backward compatibility)
- [ ] Create NEW page `/admin/marketing-signals` (don't touch old `/admin/marketing-inbox`)
- [ ] Wire new page to `trpc.marketingSignals.*` endpoints
- [ ] Add new nav link "Marketing Signals" (keep old link for now)
- [ ] Test new page works independently

### Phase B: Soft Deprecation (Later)
- [ ] Add banner to old `/admin/marketing-inbox`: "Deprecated — use Marketing Signals"
- [ ] Remove old nav link (only show "Marketing Signals")
- [ ] Monitor: confirm nobody hits old endpoints

### Phase C: Hard Delete (Much Later)
- [ ] Delete `marketingInboxRouter` from `server/routers.ts`
- [ ] Delete old UI component `AdminMarketingInbox.tsx`
- [ ] Drop `marketing_inbox_item` and `marketing_run_log` tables via migration

### Required Router Endpoints (Minimum Set)
- [x] `list` - filters + pagination
- [x] `seedTestDelawareLLCs` - generate test data
- [ ] `setStatus` - change status (new/reviewed/ignored/acted)
- [ ] `addNote` - add notes to signal

### Enterprise Design Principle
- ✅ UI is for viewing, triggering jobs, and reviewing
- ✅ Scraping/ingestion runs as background job (GCP Cloud Run / cron / agent-stack VM)
- ❌ Never block UI requests with long-running scrapes



## 🔧 DROP-IN FIX - Marketing Signals Router & UI

### Step 1: Replace Router with Drop-In Fix
- [x] Replace `/server/routers/admin/marketingSignals.ts` with user's complete implementation
- [x] Fix `seed` endpoint to call `seedTestDelawareLLCs(db, { count })` with proper signature
- [x] Ensure `list` returns `{ ok: true, rows }` with proper filtering
- [x] Ensure `setStatus` uses proper status enum
- [x] Ensure `addNote` appends to existing notes
- [x] Ensure `ingestDelawareLLCs` runs fire-and-forget (setImmediate)

### Step 2: Create New Marketing Signals UI
- [x] Create `/client/src/pages/admin/AdminMarketingSignals.tsx` (NEW page, don't touch old)
- [x] Wire to `trpc.marketingSignals.list.useQuery`
- [x] Wire to `trpc.marketingSignals.setStatus.useMutation`
- [x] Wire to `trpc.marketingSignals.addNote.useMutation`
- [x] Wire to `trpc.marketingSignals.seed.useMutation`
- [x] Add filters: status, search, sourceType, jurisdiction, score range
- [x] Add "Seed Test Signals" button
- [x] Display table: Created, Name, Jurisdiction, Source, Score, Status, Notes

### Step 3: Add Route & Navigation
- [x] Add route `/admin/marketing-signals` to App.tsx
- [ ] Add nav link "Marketing Signals" in admin sidebar (TODO: add to sidebar component)
- [x] Keep old "Marketing Inbox" link for now (side-by-side)

### Step 4: End-to-End Testing
- [x] Restart dev server
- [x] Visit `/admin/marketing-signals`
- [x] Click "Seed Test Signals" → verify 10 rows appear
- [x] Test search filter
- [x] Test status filter (UI works, backend filters correctly)
- [x] Test status change mutation
- [x] Test add note mutation
- [x] Verify duplicate seeds are ignored (canonical key dedupe)



## 🧠 MARKETING HYPOTHESES - Intelligence Agent (Signals → Hypotheses)

### Phase 1: Database Schema
- [x] Add `marketing_hypotheses` table to `drizzle/schema.ts`
- [x] Run manual SQL migration via `webdev_execute_sql` (CREATE TABLE + indexes)
- [x] Verify table exists and schema matches

### Phase 2: tRPC Router
- [ ] Create `server/routers/admin/marketingHypotheses.ts`
- [ ] Implement `list` endpoint (filter by status, limit)
- [ ] Implement `setStatus` endpoint (approve/reject/tested)
- [ ] Implement `generateFromSignals` endpoint (deterministic starter brain)
- [ ] Mount router in `server/routers.ts`

### Phase 3: Admin UI
- [ ] Create `/admin/marketing-hypotheses` page
- [ ] Wire `trpc.marketingHypotheses.list.useQuery`
- [ ] Wire `trpc.marketingHypotheses.setStatus.useMutation`
- [ ] Wire `trpc.marketingHypotheses.generateFromSignals.useMutation`
- [ ] Add "Generate from Signals" button (select signals → generate hypothesis)
- [ ] Display table: Title, Hypothesis, Segment, Channel, Confidence, Impact, Effort, Status
- [ ] Add Approve/Reject buttons

### Phase 4: Testing & Validation
- [ ] Test generate hypothesis from 3-5 signals

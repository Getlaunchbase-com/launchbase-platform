...
// Ingest knowledge corpus (generated by generateMarketingKnowledgeCorpus.mjs)
const corpusSftPath = path.join(RUNS, "knowledge-corpus", "corpus_sft.jsonl");
if (fs.existsSync(corpusSftPath)) {
  const corpusRows = readJsonl(corpusSftPath);
  console.log(`Found knowledge corpus: ${corpusRows.length} records`);
  for (const row of corpusRows) {
    ...
    row.meta.target_id = `tgt-${String(allSft.length + 1).padStart(5, "0")}`;
    row.meta.consolidated_at = new Date().toISOString();
    row.meta.source_pack = "knowledge-corpus";
    allSft.push(row);
  }
} else {
  console.log("No knowledge corpus found (run generateMarketingKnowledgeCorpus.mjs first)");
}

// Ingest synthetic scenarios (generated by generateSyntheticScenarios.mjs)
const syntheticPath = path.join(RUNS, "master-dataset", "synthetic_sft.jsonl");
if (fs.existsSync(syntheticPath)) {
  const syntheticRows = readJsonl(syntheticPath);
  console.log(`Found synthetic scenarios: ${syntheticRows.length} records`);
  for (const row of syntheticRows) {
    ...
    row.meta.target_id = `tgt-${String(allSft.length + 1).padStart(5, "0")}`;
    row.meta.consolidated_at = new Date().toISOString();
    row.meta.source_pack = "synthetic-scenarios";
    allSft.push(row);
  }
}
...
// Ingest knowledge corpus preference pairs
const corpusPrefPath = path.join(RUNS, "knowledge-corpus", "corpus_preference.jsonl");
if (fs.existsSync(corpusPrefPath)) {
  const corpusPrefRows = readJsonl(corpusPrefPath);
  console.log(`Found knowledge corpus preferences: ${corpusPrefRows.length} pairs`);
  for (const row of corpusPrefRows) {
    const h = hashPref(row);
    if (prefSeen.has(h)) { prefDupes++; continue; }
    prefSeen.add(h);
    row.meta = row.meta || {};
...
// Ingest knowledge corpus eval set
const corpusEvalPath = path.join(RUNS, "knowledge-corpus", "corpus_eval.jsonl");
if (fs.existsSync(corpusEvalPath)) {
  const corpusEvalRows = readJsonl(corpusEvalPath);
  console.log(`Found knowledge corpus evals: ${corpusEvalRows.length} scenarios`);
  for (const row of corpusEvalRows) {
    if (evalSeen.has(row.prompt)) continue;
    evalSeen.add(row.prompt);
    allEval.push(row);
  }

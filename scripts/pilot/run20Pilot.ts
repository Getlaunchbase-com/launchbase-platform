...
async function main() {
  ...
  const outDir = process.argv[3] || `/home/ubuntu/launchbase/runs/pilots_20run_${Date.now()}`;
  
  // Note: Each run takes ~110-125s, so 20 runs = ~30-40 minutes
  const stack = JSON.parse(readFileSync(stackPath, 'utf-8'));
  
  // Ensure output directory exists
  mkdirSync(outDir, { recursive: true });
  
  console.log('🚀 20-RUN PILOT: Web×10 + Marketing×10 (Production Mode)');
  console.log('Expected duration: ~30-40 minutes for 20 runs');
  ...
  for (const lane of lanes) {
    ...
    for (let rep = 1; rep <= reps; rep++) {
      ...
      const prefix = `run_${lane}_rep${String(rep).padStart(2, '0')}_`;
      const existing = readdirSync(outDir).filter(f => f.startsWith(prefix));
      if (existing.length > 0) {
        console.log(`\n[${lane} rep ${rep}/${reps}] ⏭️  Skipping (already completed: ${existing[0]})`);
        continue;
      }
      
      const runId = `pilot20_${lane}_${rep}_${Date.now()}`;
      ...
      try {
        ...
        // Write run JSON immediately (before any processing)
        const fileName = `run_${lane}_rep${String(rep).padStart(2, '0')}_${result.runId}.json`;
        writeFileSync(

...
export default function AdminSwarmNewRun() {
  ...
  const [intention, setIntention] = useState<"smoke_test"|"pressure_test"|"improve"|"critic">("smoke_test");
  const [role, setRole] = useState<"owner"|"it"|"dev"|"readonly">("owner");
  const [timeoutSec, setTimeoutSec] = useState(180);
  const [runBudgetSec, setRunBudgetSec] = useState(600);

  const [sourceType, setSourceType] = useState<"fixture"|"manual">("fixture");
  const [fixtureName, setFixtureName] = useState(FIXTURES[10]);
  ...
  const modelOptions = modelsQuery.data ?? [{ id: "openai/gpt-5-2" }, { id: "openai/gpt-4o" }];
  ...
  return (
    <div className="p-6 space-y-4">
      <div className="space-y-1">
        ...
        <p className="text-sm text-muted-foreground">Choose intention, model, role, duration, and source.</p>
      </div>
      ...
      <Card>
        ...
        <CardContent className="grid grid-cols-1 md:grid-cols-3 gap-3">
          ...
          <div className="space-y-1">
            ...
            <select className="w-full border rounded-md h-10 px-3" value={intention} onChange={(e) => setIntention(e.target.value as any)}>
          ...
          <div className="space-y-1">
            ...
            <select className="w-full border rounded-md h-10 px-3" value={role} onChange={(e) => setRole(e.target.value as any)}>
          ...
          <div className="space-y-1">
            ...
            <select className="w-full border rounded-md h-10 px-3" value={primaryModel} onChange={(e) => setPrimaryModel(e.target.value)}>
              {modelOptions.map((m) => (
          ...
          <div className="space-y-1">
            ...
            <select className="w-full border rounded-md h-10 px-3" value={fallbackModel} onChange={(e) => setFallbackModel(e.target.value)}>
          ...
          <div className="space-y-1">
            ...
            <Input type="number" value={runBudgetSec} onChange={(e) => setRunBudgetSec(Number(e.target.value))} />
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader><CardTitle className="text-base">Repo / Scope</CardTitle></CardHeader>
        <CardContent className="space-y-3">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            <div className="space-y-1">
              <div className="text-xs text-muted-foreground">Repo source</div>
              <select className="w-full border rounded-md h-10 px-3" value={repoSourceId ?? ""} onChange={(e) => setRepoSourceId(e.target.value ? Number(e.target.value) : null)}>
              ...
              <div className="text-xs text-muted-foreground">
                Manage sources in <Link href="/admin/swarm/repo" className="text-blue-600 hover:underline">Repo Sources</Link>.
              </div>
            </div>

            <div className="space-y-1">
              <div className="text-xs text-muted-foreground">Scope preset</div>
              <select className="w-full border rounded-md h-10 px-3" value={scopePreset} onChange={(e) => setScopePreset(e.target.value as any)}>
            ...
            <div className="space-y-1">
              ...
              <div className="flex gap-2">
                <Input type="number" value={maxFiles} onChange={(e) => setMaxFiles(Number(e.target.value))} placeholder="max files" />
                <Input type="number" value={maxBytes} onChange={(e) => setMaxBytes(Number(e.target.value))} placeholder="max bytes" />
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <div className="space-y-1">
              ...
              <Input value={fileQuery} onChange={(e) => setFileQuery(e.target.value)} placeholder="type to search paths…" disabled={!repoSourceId} />
              {fileSearch.isFetching && <div className="text-xs text-muted-foreground">Searching…</div>}
                ...
                <div className="max-h-48 overflow-auto border rounded p-2 space-y-1">
                  {fileSearch.data.map((f) => (
                    <button
                      ...
                      onClick={() => setSelectedFiles((prev) => prev.includes(f.path) ? prev : [...prev, f.path])}
                      type="button"
                    >
                      {f.path} <span className="text-xs text-muted-foreground">({f.size}b)</span>
                    </button>
                  ))}
                </div>
              ) : null}
              ...
              <div className="border rounded p-2 min-h-[120px] space-y-1">
                {selectedFiles.length === 0 ? (
                  ...
                  <div key={p} className="flex items-center justify-between text-sm">
                    ...
                    <Button type="button" variant="ghost" size="sm" onClick={() => setSelectedFiles((prev) => prev.filter(x => x !== p))}>Remove</Button>
                  </div>
                ))}
              </div>
            </div>
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          <div className="flex gap-2">
            <Button variant={sourceType === "fixture" ? "default" : "secondary"} onClick={() => setSourceType("fixture")}>Fixture</Button>
            <Button variant={sourceType === "manual" ? "default" : "secondary"} onClick={() => setSourceType("manual")}>Manual packet</Button>
          </div>

          {sourceType === "fixture" ? (
            <div className="space-y-1">
              <div className="text-xs text-muted-foreground">Fixture</div>
              <select className="w-full border rounded-md h-10 px-3" value={fixtureName} onChange={(e) => setFixtureName(e.target.value)}>
      ...
      <div className="flex gap-2">
        ...
        <Link href="/admin/swarm"><Button variant="secondary">Cancel</Button></Link>
